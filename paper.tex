...
\begin{document}
	...
	\begin{proof}
		We will first prove that $l$ is bounded above.
		\[
		\sum_{j=ln}^{kn} f(j)>n(k-l)f(ln)\ge n\epsilon f(ln)
		\]
		Using the sums asymptotic from \autoref{lem:sumfAsymp}
		\begin{align}
		1&\ge\liminf\frac{n\epsilon f(ln)}{\frac{L(n)}{\alpha+1}n^{\alpha+1}}\notag\\
		1&\ge\liminf\frac{(\alpha+1)\epsilon L(nl)l^\alpha}{L(n)}\notag\\
		\end{align}
		Using Potter's theorem (see 1.5.6 \cite{bingham1987regular})
		\begin{align}
			1\ge \liminf 2(\alpha+1)\epsilon l^{\alpha+1}\notag
		\end{align}
		Follows $l\le (\frac{1}{(\alpha+1)\epsilon})^{\frac{1}{\alpha+1}}$. Clearly $k\le l+1$. Now since $l,k$ are in some compact interval independent of $n$, we can use uniform convergence theorem to get
		\begin{align}
