...
\begin{document}
	...
	\begin{theorem}[Uniform Convergence Theorem]\label{UCT}
		...
		\[
		\frac{l(\lambda x)}{l(x)}\to 1\quad (x\to \infty)
		\]
	...
	\begin{theorem}[Potters Theorem]\label{PottersBound}
		\begin{enumerate}[label=(\roman*)]
			...
			\[
			\frac{f(y)}{f(x)}\le A\max\left\{\left(\frac{y}{x}\right)^{\rho+\delta}, \left(\frac{x}{y}\right)^{\rho+\delta}\right\} \quad (x\ge X, y\ge X)
			\]
		\end{enumerate}
	\end{theorem}
	\begin{proof}
	...
	\begin{proposition}[Karamata's Theorem (direct half)]\label{karamata}
		...
		\[
			\int_{X}^{x} t^\alpha l(t)dt\sim \frac{x^{\alpha+1}l(x)}{\alpha+1}\quad (x\to\infty)
		\]
	\end{proposition}
	\begin{proof}
	...
	\begin{theorem}\label{RVinverse}
		...
		\[
		f(g(x))\sim g(f(x))\sim x\quad (x\to \infty).
		\]
