...
\begin{document}
	...
	This paper is organized as follows. Section 2 is on notation. Section 3 first proves basic lemmas about asymptotics, then lemma that calculates $\phi$'s inverses integral and finally lemma that relates $\phi$ and $s_i$. Section 4 proves the theorem 1 and one technical lemma bounding sum of $s_i$ for small $i$'s.
	...
	\begin{itemize}
		...
		\item $S(x)=\#\{(i, j)\mid i\le j, f(i)+f(i+1)+\dots+f(j)\le x\}$ – the number of block sums below $x$.
	...
	Moser's idea in \cite{Moser_1963} can be extended to case $0<\alpha<1$. The idea is that
	\begin{align}
	f^{-1}(\frac{x}{k})-k\le s_k(x)\le f^{-1}(\frac{x}{k})
	\end{align}
	\begin{lemma}
		We have following asymptotic
		\begin{align}
		\sum_{i=1}^{n} f^{-1}(\frac{x}{i})\sim \frac{\alpha}{(1-\alpha)(\alpha+1)^{\frac{1}{\alpha}}}n^{1+\frac{1}{\alpha}}\frac{\tilde{L}(x)}{\tilde{L}(\frac{x}{n})}
		\end{align}
		where $x^{\frac{1}{\alpha}}\tilde{L}(x)$ is the asymptotic inverse of $f$.
	\end{lemma}
	\begin{proof}
		Regularly varying functions inverse is regularly varying with index $\frac{1}{\alpha}$ (see Theorem 1.5.12 in \cite{bingham1987regular}, or Theorem~\ref{RVinverse} in the Appendix). Let $f^{-1}(x)=x^{\frac{1}{\alpha}}\tilde{L}(x)$.
		\begin{align}
		\sum_{i=1}^{n} f^{-1}(\frac{x}{i})\sim \int_{1}^{n} f^{-1}(\frac{x}{t})dt=x^{\frac{1}{\alpha}}\int_{1}^{n} t^{-\frac{1}{\alpha}}\tilde{L}(\frac{x}{t})
		\end{align}
		By doing change of variable $u=\frac{x}{t}$ we get
		\begin{align}
			x\int_{\frac{x}{n}}^{x} u^{\frac{1}{\alpha}-2} \tilde{L}(u)du
		\end{align}
		Using Karamata's theorem (see proposition 1.5.8 of \cite{bingham1987regular})
		\begin{align}
			x\int_{\frac{x}{n}}^{x} u^{\frac{1}{\alpha}-2} \tilde{L}(u)du
			&\sim
			\frac{\alpha}{1-\alpha}x^{\frac{1}{\alpha}}(\tilde{L}(x)-n^{1-\frac{1}{\alpha}}\tilde{L}(\frac{x}{n}))
			\\
			&\to \frac{\alpha}{1-\alpha}x^{\frac{1}{\alpha}}\tilde{L}(x)\quad (n\to\infty)
		\end{align} 
		Since
		\begin{align}
			\frac{1}{\alpha}&>\frac{2}{\alpha+1}\\
			\alpha&<1
		\end{align}
		it follows that $n^2=o(\sum_{i=1}^{n} f^{-1}(\frac{x}{i}))$. 
		...
		Thus
		\begin{align}
			\frac{\alpha}{1-\alpha}x^{\frac{1}{\alpha}}\tilde{L}(x)
			&\sim \frac{\alpha}{(1-\alpha)(\alpha+1)^{\frac{1}{\alpha}}}n^{1+\frac{1}{\alpha}}L(n)^{\frac{1}{\alpha}}\tilde{L}(\frac{x}{n})\frac{\tilde{L}(x)}{\tilde{L}(\frac{x}{n})}\\
			&\sim
			\frac{\alpha}{(1-\alpha)(\alpha+1)^{\frac{1}{\alpha}}}n^{1+\frac{1}{\alpha}}\frac{\tilde{L}(x)}{\tilde{L}(\frac{x}{n})}
		\end{align}
	\end{proof}
