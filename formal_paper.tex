	...
	\begin{document}
		...
		\begin{abstract}
			Interval sum is sum of consecutive terms in a sequence. An asymptotic formula is found for the number of interval sums of a class of sequences. Namely $\left(f(i)\right)_{i=1}^{\infty}$, where $f$ is measurable, locally bounded away from 0 and $\infty$ and eventually monotonic with index $\alpha> 1$. This extends results of O'Sullivan et al. on interval sums of prime powers. 
		...
		\begin{corollary}\label{asympUpperBound}
			The set of of distinct interval sums
			\begin{align}
				S'(x)=\left\{y\mid \sum_{n=i}^{j} f(n)=y<x\right\}
			\end{align}
			has inequality $|S'(x)|\lesssim n^2C(\alpha)$. 
		\end{corollary}
		\begin{proof}
			Since $|S'(x)|\leq S(x)$, this is direct consequence of theorem~\ref{BlockSumsforAlphaGr1}.
		\end{proof}
		
		\begin{corollary}
			If furthermore $n\in \mathbb{N}\implies f(n)\in\mathbb{N}$, then natural density of $S'=\bigcup_{x=1}^{\infty} S'(x)$ is $0$.
		\end{corollary}
		\begin{proof}
			...
			\end{align}
			Using corollary~\ref{asympUpperBound} and lemma~\ref{sum f Asymp} we get
			\begin{align}
				\frac{S'(x)}{x}\lesssim \frac{n^2C(\alpha)}{n^{1+\alpha}/(\alpha+1)}=O(n^{1-\alpha}).
			\end{align}
			Since $1-\alpha<0$ the claim follows.
		\end{proof}
		...
		\section{Use of AI}
		AI has been used in making of this article. AI was used to finalize writing, proof checking and in finding u-substitution ideas to solve the integral in lemma~\ref{phi integral}. The AI models used were GPT-5 mini, GPT-5.1, Gemini 3 Flash and Gemini 3 Pro. Gemini was only used for writing assistance and GPT was also used for proof checking and u-substitution ideas. Everything AI produced was thoroughly checked. 
