...
\begin{document}
	...
	\begin{proof}
		...
		\begin{align}
			x\sim\frac{1}{\alpha+1}L(n)n^{\alpha+1}&=\label{eq:yap1} \\
		...
		\end{equation}
		Though this assumes that the inversion from $x\sim \sum_{i=nl}^{nk} f(i)$ to $s_{\lambda n, f}(x) \sim nl$ is valid. To prove that this inversion is valid, note that the steps through \ref{eq:yap1} to \ref{eq:yap2} can be reversed and $(t+\lambda)^{\alpha+1}-t^{\alpha+1}$ is strictly monotone.
	\end{proof}
