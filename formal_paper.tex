...
\begin{document}
	...
	\textbf{Interval sum} is the sum of consecutive terms in a sequence. Determining the asymptotic behavior of the count of interval sums less than $x$ has been investigated for primes and prime powers \cite{Moser_1963, primePowerConsecutiveSums, primeSquaresConsecutiveSums}. Moser \cite{Moser_1963} determined the asymptotic behavior of interval sums of primes. Tongsomporn et al. \cite{primeSquaresConsecutiveSums} derived  bounds for the number of interval sums of prime squares, though the bounds were not asymptotic to each other. O'Sullivan et al. \cite{primePowerConsecutiveSums} found asymptotic upper and lower bounds for the number of interval sums of prime powers.
	...
	That is
	\begin{equation}
		S_f(x)\sim n_f(x)^2C(\alpha) \qquad x\to\infty
	\end{equation}
	where $n:=n_f(x)$ is the length of the largest interval, with following asymptotic relation to $x$, $x\sim \frac{nf(n)}{\alpha+1}, x\to\infty$ and 
	\begin{equation}
	...
	This paper is organized as follows. Section \ref{notation} defines our notation and some global assumptions. Section \ref{preliminaries} covers preliminaries and one lemma. Section \ref{mainTheorem} proves a lemma that relates $\phi$ and $s_{i, f}$, a lemma that calculates the integral of the inverse of $\phi$, one technical lemma bounding the sum of $s_{i, f}$ for small $i$ and finally proves the main theorem.
	...
	\begin{itemize}
		...
		\begin{equation}
			n_f(x)=\max\left\{n\in \mathbb{N} \mid \sum_{i=1}^{n} f(i)\leq x\right\}\label{def:n}
		\end{equation}
	...
	\begin{lemma}\label{sum f Asymp}
		...
		\end{equation}
		for $\alpha>-1$. Note $n:=n_f(x)$ (see notation \ref{def:n}).
	\end{lemma}
	\begin{proof}
		From Karamata's theorem (see proposition 1.5.8 in \cite{bingham1987regular} or Proposition~\ref{karamata} in the appendix)
		\begin{align}
			\int_{1}^{n} f(i)\sim \frac{nf(n)}{\alpha+1}.
		\end{align}
		From eventual monotonicity and regular variation with index $\alpha>-1$ of $f$ it follows that
		\begin{align}
			\sum_{i=1}^{n} f(i)\sim\int_{1}^{n} f(i)
		\end{align}
		From definition of $n=n_f(x)$ the result follows.
	\end{proof}
	...
	\begin{lemma}\label{l,k paramtrization}
		Let $\epsilon>0$ and $1-\epsilon\geq\lambda\geq\epsilon$. Let $l=\phi^{-1}(\lambda)$ and $k=l+\lambda$, then uniformly in $\lambda$
		\begin{align}
			x\sim \sum_{i=n\phi^{-1}(\lambda)}^{n(\phi^{-1}(\lambda)+\lambda)} f(i)
		\end{align}
		where $l$ is unique and $\phi(x)=(1+x^{\alpha+1})^{\frac{1}{\alpha+1}}-x$.
	\end{lemma}
	
	\begin{proof}
		We are going to consider sums of the form $\sum_{i=ln}^{kn} f(i)$. Let $l,k>0$ be such that $k-l=\lambda$ and $k^{\alpha+1}-l^{\alpha+1}=1$. Such $k$ and $l$ exists, by monotonicity of $(l+\lambda)^{\alpha+1}-l^{\alpha+1}$. Now Using mean-value theorem we get that for some $\zeta\in(l, k)$
		\begin{equation}
		...
		\end{align}
		To show that $k,l$ are bounded below note that, if $l<\epsilon$ then
		\begin{align}
			1=(l+\lambda)^{\alpha+1}-l^{\alpha+1}<(\epsilon+\lambda)^{\alpha+1}-\epsilon^{\alpha+1}=1-\epsilon^{\alpha+1}<1
		\end{align}
		which is a contradiction.
		
		Now $k$ and $l$ are in a compact subset of $(0,\infty)$, thus we can use the uniform convergence theorem (see Theorem 1.2.1 in \cite{bingham1987regular} or Theorem~\ref{UCT} in the Appendix). Using uniform convergence theorem and lemma~\ref{sum f Asymp}
		\begin{align}
			x\sim\frac{1}{\alpha+1}L(n)n^{\alpha+1}&= \label{eq:yap0}\\
			\frac{1}{\alpha+1}L(n)n^{\alpha+1}(k^{\alpha+1}-l^{\alpha+1})&=\label{eq:yap1}\\
			L(n)\int_{nl}^{nk} t^\alpha dt&\sim\\
			L(n)\sum_{i=nl}^{nk} i^\alpha&=\\
			\sum_{i=nl}^{nk} i^\alpha L(n) &\sim \sum_{i=nl}^{nk} f(i).\label{eq:yap2}
		\end{align}
		...
		\begin{align}
			...
			l&=\phi^{-1}(\lambda)
		\end{align}
		In lemma~\ref{phi integral} it is proven that the inverse of $\phi$ is well defined. The $l$ is unique, because otherwise $k^{\alpha+1}-l^{\alpha+1}\neq 1$, thus same reasoning as \ref{eq:yap0} to \ref{eq:yap2} gives a contradiction.
		
	\end{proof}
	
	\begin{lemma}\label{s asymp}
		Let $\epsilon>0$ and $1-\epsilon\geq\lambda\geq\epsilon$, then $s_{\lambda n, f}(x)$ has the following asymptotic uniformly
		\begin{equation}
	...
	\begin{proof}
		Since $s_{\lambda n, f}(x)$ counts the number of intervals of length $\lambda n$, we get from from lemma~\ref{l,k paramtrization}
		\begin{equation}
			s_{\lambda n, f}(x) \sim n\phi^{-1}(\lambda).
		\end{equation}
		
		
		We will prove uniformity of the asymptotic by contradiction. Assume that it is not uniform then there exists $\epsilon_0>0$ such that for all $X_0$ there exists $x>X_0$ and $\lambda_x\in[\epsilon,1-\epsilon]$ such that 
		\begin{align}\label{uniformContradictory}
			\abs{\frac{s_{\lambda_x n, f}(x)}{n\phi^{-1}(\lambda_x)}-1}>\epsilon_0
		\end{align}
		Now take sequence $(x_m, \lambda_m)$ that satisfies \ref{uniformContradictory}. There are two cases\\
		\textbf{Case 1:} $s_{\lambda n, f}(x)>n\phi^{-1}(\lambda)(1+\epsilon_0)$\\
		It would follow that
		\begin{align}
			x\sim \sum_{i=s_{\lambda n, f}(x)}^{s_{\lambda n, f}(x)+n\lambda} f(i)>\sum_{i=n\phi^{-1}(\lambda)(1+\epsilon_0)}^{n(\phi^{-1}(\lambda)(1+\epsilon_0)+\lambda)} f(i)
		\end{align}
		We also have by same argument as steps from \ref{eq:yap2} to \ref{eq:yap1} that
		\begin{align}
			\sum_{i=n\phi^{-1}(\lambda)(1+\epsilon_0)}^{n(\phi^{-1}(\lambda)(1+\epsilon_0)+\lambda)} f(i)\sim x((\phi^{-1}(\lambda)(1+\epsilon_0)+\lambda)^{\alpha+1}-((\phi^{-1}(\lambda)(1+\epsilon_0))^{\alpha+1})
		\end{align}
		We know that 
		\begin{align}
			\left(\phi^{-1}(\lambda)(1+\epsilon_0)+\lambda\right)^{\alpha+1}-\left(\phi^{-1}(\lambda)(1+\epsilon_0)\right)^{\alpha+1}=1+F_+(\lambda)
		\end{align}
		for some continuous $F_+$ and that $F_+(\lambda)>0$, hence it has minimum $c>0$ in the compact $\lambda$-set. Thus independent $\lambda$ we get that
		\begin{align}
			x\gtrsim x(1+c)
		\end{align}
		which is a contradiction.
		\\
		\\
		\textbf{Case 2:} $s_{\lambda n, f}(x)<n\phi^{-1}(\lambda)(1-\epsilon_0)$\\
		It would follow that
		\begin{align}
			x\sim \sum_{i=s_{\lambda n, f}(x)}^{s_{\lambda n, f}(x)+n\lambda} f(i)<\sum_{i=n\phi^{-1}(\lambda)(1-\epsilon_0)}^{n(\phi^{-1}(\lambda)(1-\epsilon_0)+\lambda)} f(i)
		\end{align}
		Where inequality comes from monotonicity of $f$. We also have by same argument as steps from \ref{eq:yap2} to \ref{eq:yap1} that
		\begin{align}
			\sum_{i=n\phi^{-1}(\lambda)(1-\epsilon_0)}^{n(\phi^{-1}(\lambda)(1-\epsilon_0)+\lambda)} f(i)\sim x((\phi^{-1}(\lambda)(1-\epsilon_0)+\lambda)^{\alpha+1}-((\phi^{-1}(\lambda)(1-\epsilon_0))^{\alpha+1})
		\end{align}
		We know that 
		\begin{align}
			\left(\phi^{-1}(\lambda)(1-\epsilon_0)+\lambda\right)^{\alpha+1}-\left(\phi^{-1}(\lambda)(1-\epsilon_0)\right)^{\alpha+1}=1-F(\lambda)
		\end{align}
		for some continuous $F_-$ and that $F_-(\lambda)>0$, hence it has minimum $c>0$ in the compact $\lambda$-set. Thus independent $\lambda$ we get that
		\begin{align}
			x\lesssim x(1-c)
	...
	\begin{lemma}\label{head's neglible}
		Let $\epsilon>0$, $\alpha>1$, and $0\leq l\leq1-\epsilon$ then 
		\begin{equation}
			\sum_{i=nl}^{n(l+\epsilon)} s_{i, f}(x)=O(n^2)\epsilon^{\frac{1}{2}(1-\frac{1}{\alpha})},
		\end{equation}
	...
	\begin{proof}
		...
		it follows that 
		\begin{align}
			s_{k, f}(x)\le f^{-1}\left(\frac{x}{k}\right).\label{invUpperbound}
		\end{align} 
		The inverse of a regularly varying function is regularly varying with index $\frac{1}{\alpha}$ (see Theorem 1.5.12 of \cite{bingham1987regular} or Theorem~\ref{RVinverse} in the Appendix). Let $f^{-1}(x)=x^{\frac{1}{\alpha}}\tilde{L}(x)$. 
		We get from monotonicity of $f$ that
		\begin{equation}
			\sum_{i=nl}^{n(l+\epsilon)} f^{-1}\left(\frac{x}{i}\right)\sim \int_{nl}^{n(l+\epsilon)} f^{-1}\left(\frac{x}{t}\right)dt=x^{\frac{1}{\alpha}}\int_{nl}^{n(l+\epsilon)} t^{-\frac{1}{\alpha}}\tilde{L}\left(\frac{x}{t}\right)dt.
		\end{equation}
		...
		\begin{align}
			x^{\frac{1}{\alpha}}\int_{nl}^{n(l+\epsilon)} t^{-\frac{1}{\alpha}}\tilde{L}\left(\frac{x}{t}\right)dt&\le
			x^{\frac{1}{\alpha}}M\tilde{L}\left(\frac{x}{n}\right)n^\delta\int_{nl}^{n(l+\epsilon)} t^{-\frac{1}{\alpha}-\delta}.
		\end{align}
		By definition
		\begin{align}
			f^{-1}(f(n))&\sim n\\
		...
		\begin{align}
			&x^{\frac{1}{\alpha}}M\tilde{L}\left(\frac{x}{n}\right)n^\delta\int_{nl}^{n(l+\epsilon)} t^{-\frac{1}{\alpha}-\delta}\\
			\sim &Mn^{2}L(n)^{\frac{1}{\alpha}}\tilde{L}(n^\alpha L(n))\frac{2\alpha}{\alpha-1}((l+\epsilon)^{\frac{1}{2}(1-\frac{1}{\alpha})}-l^{\frac{1}{2}(1-\frac{1}{\alpha})})\\
			\sim &Mn^{2}\frac{2\alpha}{\alpha-1}((l+\epsilon)^{\frac{1}{2}(1-\frac{1}{\alpha})}-l^{\frac{1}{2}(1-\frac{1}{\alpha})})\\
			=&O(n^2)((l+\epsilon)^{\frac{1}{2}(1-\frac{1}{\alpha})}-l^{\frac{1}{2}(1-\frac{1}{\alpha})})\\
	...
	\begin{theorem}\label{BlockSumsforAlphaGr1}
		Under the assumptions and notation in \ref{notation}, the function $S_f(x)$ has the asymptotic $n_f(x)^2 C(\alpha), x\to\infty$ for $\alpha>1$, where 
		\begin{equation}
			C(\alpha)=\frac{1}{2(\alpha+1)}
			\frac{\Gamma(\frac{1}{\alpha+1})\Gamma(\frac{\alpha-1}{\alpha+1})}{\Gamma(\frac{\alpha}{\alpha+1})}.
		\end{equation}
	...
	\begin{proof}
		...
		\begin{equation}\label{sumSplit}
			S_f(x)=\sum_{i=1}^{n\epsilon} s_{i, f}(x)+\sum_{i=n\epsilon}^{n(1-\epsilon)} s_{i, f}(x)+\sum_{i=n(1-\epsilon)}^{n} s_{i, f}(x).
		\end{equation}
		Using Lemma~\ref{s asymp} and continuity of $\phi^{-1}$, we get
		\begin{align}
			\sum_{i=n\epsilon}^{n(1-\epsilon)} s_{i, f}(x)&\sim \sum_{i=n\epsilon}^{n(1-\epsilon)} n\phi^{-1}\left(\frac{i}{n}\right)\\
			&=n\sum_{i=n\epsilon}^{n(1-\epsilon)} \phi^{-1}\left(\frac{i}{n}\right)\\
			&\sim n \int_{n\epsilon}^{n(1-\epsilon)} \phi^{-1}\left(\frac{t}{n}\right)dt\\
			&=n^2\int_{\epsilon}^{1-\epsilon} \phi^{-1}(t)dt.
		\end{align}
		Since the integral converges
		\begin{equation}
			\lim_{\epsilon\to0^+} \int_{\epsilon}^{1-\epsilon} \phi^{-1}(t)dt =\int_{0}^{1} \phi^{-1}(t)dt
		\end{equation}
		From Lemma~\ref{head's neglible}, it follows that the first and third sum in \ref{sumSplit} are negligible in the asymptotic, hence taking $\epsilon\to 0$ finishes the proof. 
	\end{proof}
	...
	Figure~\ref{SVeffectsOnConvergence} shows the impact of the slowly varying part. We can observe that larger growth rate in the slowly varying part corresponds to slower convergence.
		\begin{figure}[H]
		\centering
		\begin{tikzpicture}[scale=0.9]
			\begin{axis}
	...
	\end{figure}
		Figure \ref{indexEffectsOnConvergence} shows the impact of the index on convergence. We can observe that increasing the index slows down the convergence.
