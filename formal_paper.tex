...
\begin{document}
	...
	\section{case $\alpha<1$}\label{minorTheorem}
	Moser's idea in \cite{Moser_1963} can be extended to case $0<\alpha<1$. The idea is that
	\begin{align}
		f^{-1}\left(\frac{x}{k}\right)-k\le s_k(x)\le f^{-1}\left(\frac{x}{k}\right).
	\end{align}
	\begin{theorem}
		We have following asymptotic
		\begin{align}
			S(x)\sim \frac{\alpha}{(1-\alpha)(\alpha+1)^{\frac{1}{\alpha}}}n^{1+\frac{1}{\alpha}}\frac{\tilde{L}(x)}{\tilde{L}(\frac{x}{n})}
		\end{align}
		where $x^{\frac{1}{\alpha}}\tilde{L}(x)$ is the asymptotic inverse of $f$ and $f\in RV_\alpha^+$ with $0<\alpha<1$.
	\end{theorem}
	\begin{proof}
