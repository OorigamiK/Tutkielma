...
\begin{document}
	...
	\begin{proof}
		...
		\begin{align}
			...
			l&=\phi^{-1}(\lambda)
		\end{align}
		In lemma~\ref{phi integral} it is proven that the inverse of $\phi$ is well defined. The value of $l$ is unique, because otherwise $k^{\alpha+1}-l^{\alpha+1}\neq 1$, thus same reasoning as \ref{eq:yap0} to \ref{eq:yap2} gives a contradiction.
	...
	\begin{proof}
		...
		\end{align}
		for some continuous $F$ and that $F(\lambda)>0$, hence it has minimum $c>0$ in the compact $\lambda$-set. Thus independent $\lambda$ we get that
		\begin{align}
			x\gtrsim x(1+c)
		\end{align}
		which is a contradiction.
		...
		This is same argument as case 1.
	\end{proof}
