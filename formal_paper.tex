...
\begin{document}
	...
	\begin{itemize}
		...
		\item $S(x)=\#\{(i, j)\in\mathbb{N}^2\mid i\le j, f(i)+f(i+1)+\dots+f(j)\le x\}$ – the number of interval sums below $x$.
		\item $s_k(x)=\#\{(i, j)\in\mathbb{N}^2\mid j-i=k-1, f(i)+f(i+1)+\dots+f(j)\le x\}$ – the number of 	interval sums of length $k$ below $x$.
